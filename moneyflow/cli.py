...
@click.option('--year', type=int, metavar='YYYY',
              help='Only load transactions from this year onwards (e.g., --year 2025)')
@click.option('--since', type=str, metavar='YYYY-MM-DD',
              help='Only load transactions from this date onwards (overrides --year)')
@click.option('--mtd', is_flag=True,
              help='Load month-to-date transactions (from 1st of current month)')
@click.option('--cache', type=str, metavar='PATH',
              help='Enable caching. Optionally specify cache directory')
@click.option('--refresh', is_flag=True,
              help='Force refresh from API, skip cache even if valid')
